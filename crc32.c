...
/**
  * @brief  Function of calculating crc32.
  * @param  p_data: Pointer to a unsigned char type, data buffer to be calculated.
  * @param  size: Length of p_data.
  * @retval crc32 value.
  */
unsigned int crc32_calc(unsigned char * p_data, unsigned int size, unsigned int * p_crc)
{
#if CRC32_USE_TABLE
  return generate_crc32c(p_data, size);
#else
  unsigned int crc;
